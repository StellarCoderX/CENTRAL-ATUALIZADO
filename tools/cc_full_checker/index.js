// /tools/cc_full_checker/index.js

export function render(appRoot) {
  document.title = "Checker CC FULL | Central de Checkers Pro";
  ...
  appRoot.innerHTML = `
    ...
    <div class="checker-container cyber-fade-in">
      <div class="main-card cyber-card">
        <div class="header">
          <div class="logo">
              <div class="logo-icon cyber-icon"><i class="fas fa-credit-card"></i></div>
              <h1 class="logo-text cyber-title">Checker CC FULL</h1>
          </div>
          <p class="subtitle terminal-text">Envie um arquivo .txt para testar.</p>
        ...
        <div class="cyber-info-panel">
            <p class="terminal-text" style="text-align: center;">
                ...
                <span style="color: var(--success);">Aprovada (Live):</span> 50 créditos<br>
                <span style="color: var(--warning);">Saldo Insuficiente:</span> 20 créditos<br>
                <span style="color: var(--danger);">Reprovada:</span> 0 créditos
            </p>
        </div>

        <form id="cc-full-form">
...
function initCcFullLogic() {
  const form = document.getElementById("cc-full-form");
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      const response = await fetch("/api/tools/cc_full_checker", {
        method: "POST",
        headers: headers,
        body: formData,
      });
