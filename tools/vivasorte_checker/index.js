// /tools/shein_checker/index.js (Versão Simplificada sem Proxy)

export function render(appRoot) {
  document.title = "Checker Shein | Central de Checkers Pro";

  // Estilos (CSS) da ferramenta (mantidos como estão)
  const toolStyle = `
    :root {
...
// Lógica principal do checker, agora sem código de proxy
function initCheckerLogic() {
  let total = 0, tested = 0, lives = 0, dies = 0, errors = 0;
  ...
  async function testar(lista) {
    ...
    const ajaxData = { lista: conteudo };
    const statusMessage = `<span class="loading-spinner"></span> Testando: ${conteudo}`;
